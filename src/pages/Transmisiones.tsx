...
export default function Transmisiones() {
  ...
  const { isLive, remaining, zoomLink } = useLiveService()
  ...
  useEffect(() => {
    ...
    const cargarVideosAnteriores = async () => {
      try {
        ...
        console.error('Error al cargar la lista de reproducción de YouTube:', error)
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-gray-900 dark:to-slate-950 py-16">
      <div className="container max-w-6xl">
        ...
        <div className="text-center mb-12">
          ...
          {/* Temporizador de horario de cultos */}
          <div className="mt-6 flex justify-center">
            <span
              className={
                'inline-flex items-center gap-2 px-6 py-3 rounded-full font-semibold shadow-lg transition-all ' +
                (isLive ? 'bg-emerald-500 text-white animate-pulse' : 'bg-emerald-600/90 text-white')
              }
            >
              {isLive ? '¡EN VIVO AHORA!' : remaining ? `En Vivo en ${remaining}` : 'En Vivo'}
            </span>
          </div>
        </div>
        ...
        {transmisionActiva ? (
          ...
          <div className="mb-12 animate-in fade-in slide-in-from-top duration-500">
            <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 p-10 md:p-12 text-center shadow-2xl">
              <div className="flex items-center justify-center gap-2 mb-4">
                <div className="w-3 h-3 bg-white rounded-full animate-pulse"></div>
                <span className="text-white font-semibold text-sm tracking-wide uppercase">En vivo ahora</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-light text-white mb-4">
                ¡El culto ya comenzó!
              </h2>
              <p className="text-emerald-50 font-light mb-8 max-w-md mx-auto">
                Únete ahora mismo a la transmisión por Zoom y adoremos juntos.
              </p>
              <a
                href={zoomLink}
                target="_blank"
                rel="noreferrer"
                className="inline-block px-10 py-4 rounded-full bg-white text-emerald-700 font-bold text-lg shadow-lg hover:scale-105 transition-transform animate-pulse"
              >
                🔴 Unirme al Zoom
              </a>
            </div>
          ...
          <div className="text-center mb-10">
            ...
            {!videosLoading && !videosError && (
              <p className="text-sm text-gray-600 dark:text-gray-400 font-light">
                {videos.length} grabaciones disponibles
              </p>
            )}
          </div>
          ...
          {videosLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-gray-200/50 dark:border-gray-800/50 overflow-hidden animate-pulse"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {videos.map((video) => (
                <a
                  ...
                  className="group bg-white/60 dark:bg-gray-900/40 backdrop-blur-xl rounded-xl border border-gray-200/50 dark:border-gray-800/50 overflow-hidden hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                >
                  {/* Thumbnail */}
                  ...
                  {/* Content */}
                  <div className="p-5">
                    <h3 className="font-light text-lg text-gray-900 dark:text-white mb-2 line-clamp-2">
                      {video.title}
                    </h3>
